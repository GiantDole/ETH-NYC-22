...
class Web3WalletPage extends StatelessWidget {
  const Web3WalletPage({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => getIt<WalletConnectBloc>(),
          ...
          body: Container(
            ...
            child: Column(
              children: [
                ...
                if (state.connected) ...[
                  Text("Address:" + state.sessionStatus!.accounts.toString()),
                  DPPrimaryButton(
                      text: "Sign Message",
                      onPressed: () => context.read<WalletConnectBloc>().sign(
                            message: "test",
                          ))
                ],
                DPPrimaryButton(
  ...
  Future<void> connectWallet() async {
    ...
    if (!connector.connected) {
      sessionStatus = await connector.createSession(
        ...
        onDisplayUri: (uri) {
          launchUrlString(uri); //call the launchUrl(uri) method
        },
      );
    }
  }
...
//   @override
//   Widget build(BuildContext context) {
//     return MyHomePage(title: 'Wallet Connect');
//   }
// }
...
//                   context.router.push(QRScanViewRoute()).then((value) {
//                     if (value != null) {
//                       _qrScanHandler(value as String);
//                     }
//                   });
//                   break;
